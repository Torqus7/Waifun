...
async function editAndSendImage(bot, message, arg, image){
    let img = `./assets/gmedits/test/cycle${message.author.id}.jpg`;

    let number = 1;
    if(!arg) number = 100;
    else number = await parseInt(arg, 10)
    if(isNaN(number)) return message.reply(`You didn't input a number. Correct syntax ${bot.prefix}cycle, or ${bot.prefix}cycle <number>`);
